# Digital Twin — Technical Specification

## Purpose

Technical implementation guide for **Digital Twin** in the monorepo.
...
|-------|----------------|
| Routes | `apps/web/app/...` |
| Feature code | `apps/web/features/digital-twin/` |
| Shared UI | `@repo/ui` |
...
- Server Action or Route Handler delegates to `@repo/ai` gateway
- Streaming via `ReadableStream` to client chat hook
- Citations rendered from `citation` stream events
...
- Export public API from `features/digital-twin/index.ts`
