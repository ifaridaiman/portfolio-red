...
Technical implementation guide for **Articles** in the monorepo.
...
|-------|----------------|
| Routes | `apps/web/app/...` |
| Feature code | `apps/web/features/articles/` |
| Shared UI | `@repo/ui` |
...
- MDX or rich text stored in PostgreSQL
- ISR/SSG with `revalidateTag` on admin publish
- Triggers RAG ingestion on publish
...
- Export public API from `features/articles/index.ts`
